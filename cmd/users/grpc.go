...
import (
	...
	rolessvr "users/gen/grpc/roles/server"
	userspb "users/gen/grpc/users/pb"
	userssvr "users/gen/grpc/users/server"
...
func handleGRPCServer(ctx context.Context, u *url.URL, usersEndpoints *users.Endpoints, rolesEndpoints *roles.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	)
	{
		...
		rolesServer = rolessvr.New(rolesEndpoints, nil)
	}
	...
	rolespb.RegisterRolesServer(srv, rolesServer)
